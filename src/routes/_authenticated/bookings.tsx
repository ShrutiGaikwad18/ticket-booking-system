              ...
              <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-4 sm:flex sm:items-center sm:justify-between">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 text-[10px] uppercase tracking-wider">
                    <span className={`rounded-full px-2 py-0.5 ${b.status === "confirmed" ? "bg-[color:var(--color-success)]/20 text-[color:var(--color-success)]" : "bg-[color:var(--color-destructive)]/20 text-[color:var(--color-destructive)]"}`}>
                      {b.status}
                    </span>
                    <span className="text-muted-foreground">Booked {format(new Date(b.created_at), "dd MMM yyyy")}</span>
                  </div>
                  <h3 className="mt-2 truncate text-lg font-semibold">{ev?.title}</h3>
                  <div className="mt-1 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                    {b.show && <span className="flex items-center gap-1"><Calendar className="h-3 w-3" />{format(new Date(b.show.starts_at), "EEE, dd MMM · h:mm a")}</span>}
                    {ev?.venue && <span className="flex items-center gap-1"><MapPin className="h-3 w-3" />{ev.venue.name}</span>}
                  </div>
                  <div className="mt-2 text-xs text-muted-foreground">
                    Seats: {b.booking_seats.map((bs: { seat: { row_label: string; seat_number: number } }) => `${bs.seat.row_label}${bs.seat.seat_number}`).join(", ")}
                  </div>
                </div>
                <div className="flex flex-col items-end gap-2 shrink-0">
                  ...
                  <div className="flex gap-2">
                    <Button asChild size="sm" variant="outline">
                      <Link to="/bookings/$id" params={{ id: b.id }}>View ticket</Link>
                    </Button>
                    {b.status === "confirmed" && (
                      <Button size="sm" variant="ghost" onClick={() => { if (confirm("Cancel this booking?")) cancel.mutate(b.id); }}>
                        Cancel
                      </Button>
                    )}
                  </div>
